...
cohen.d(data_docs$age, data_docs$sentim_final)
cohen.d(data_docs$rasch_score_total, data_docs$sentim_final)
cohen.d(data_docs$trust_score, data_docs$sentim_final)
cohen.d(data_docs$stofhlascore, data_docs$sentim_final)

# Standardized difference for categorical 
stddiff.category(test, data_docs$sentim_final, data_docs$sex)
# some lapply way of doing it? 
...
# Combined Table 1
tab1_combined <- cbind(summary_table(data_docs, summary1), summary_table(dplyr::group_by(data_docs, sentim_final), summary1))
...
# Print
print(tab1_combined, 
      cnames = c("Negative (n=42)", "Positive (n=40)", "Combined (n=82)"), 
...
#------------------------
tableone <- print(tab1_combined, 
      cnames = c("Negative (n=42)", "Positive (n=40)", "Combined (n=82)"), 
      rtitle = "Demographics of Respondents with Family Members pf COPD patients Dependent on Home O2 - Comments About Doctors by Sentiment, As Categorized by Humans (Gold Standard)")

write.table(tab1_combined, file = "test.txt")
